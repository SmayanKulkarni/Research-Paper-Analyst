...
def verify_citation_integrity(claim_text: str, reference_string: str):
    ...
    Returns:
        A report containing the real abstract of the cited paper and validation metadata.
    """
    # 1. Search for the real paper
    result = citation_verifier.verify_claim(claim_text, reference_string)
    
    if result["status"] != "found":
        return f"Could not verify. Reason: {result.get('reason')}"
        
    # 2. Return the abstract so the Agent can compare
    return (
        f"--- CITATION DATA ---\n"
